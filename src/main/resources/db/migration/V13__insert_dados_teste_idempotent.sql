-- V13: Insert sample/test data (idempotent - will not duplicate existing rows)

-- Exemplos
INSERT INTO exemplos (id, nome, numero, data_horario_criado, exemplo_enum, ativo)
SELECT 1, 'Exemplo A1', 10, NOW() - INTERVAL '15 day', 'A', TRUE
WHERE NOT EXISTS (SELECT 1 FROM exemplos WHERE id = 1);

INSERT INTO exemplos (id, nome, numero, data_horario_criado, exemplo_enum, ativo)
SELECT 2, 'Exemplo B1', 20, NOW() - INTERVAL '12 day', 'B', TRUE
WHERE NOT EXISTS (SELECT 1 FROM exemplos WHERE id = 2);

INSERT INTO exemplos (id, nome, numero, data_horario_criado, exemplo_enum, ativo)
SELECT 3, 'Exemplo C1', 30, NOW() - INTERVAL '9 day', 'C', TRUE
WHERE NOT EXISTS (SELECT 1 FROM exemplos WHERE id = 3);

INSERT INTO exemplos (id, nome, numero, data_horario_criado, exemplo_enum, ativo)
SELECT 4, 'Exemplo D1', 40, NOW() - INTERVAL '6 day', 'D', TRUE
WHERE NOT EXISTS (SELECT 1 FROM exemplos WHERE id = 4);

INSERT INTO exemplos (id, nome, numero, data_horario_criado, exemplo_enum, ativo)
SELECT 5, 'Exemplo E1', 50, NOW() - INTERVAL '3 day', 'E', TRUE
WHERE NOT EXISTS (SELECT 1 FROM exemplos WHERE id = 5);
...
-- Estabelecimentos (ensure referenced endereco and usuario exist)
...
SELECT setval('exemplos_id_seq', (SELECT COALESCE(MAX(id), 0) FROM exemplos));
